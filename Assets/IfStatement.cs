...
public class IfStatement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            Roof.GetComponent<SpriteRenderer>().color = Color.red;
    ...
    private void OnMouseDown()
    {
        ...
        if (AddToStatement && !ActiveAdd || ListStatement.AddToStatement)
        {
        ...
        for (int x = 0; x < IsSelect.Length; x++)
        {
            try
            {
                if (IsSelect[x].GetComponent<IfStatement>().Selected)
                {
                    IsSelect[x].GetComponent<IfStatement>().Selected = false;
                }
            } catch { }
        }
        ...
        if (IsDemo)
        {
            IfStatement CreateIfStatemnt = Instantiate(gameObject).GetComponent<IfStatement>();
